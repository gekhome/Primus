...
{
    public class ApofasiStegasiService : IApofasiStegasiService, IDisposable
    {
        private readonly PrimusDBEntities entities;

        public ApofasiStegasiService(PrimusDBEntities entities)
        {
            this.entities = entities;
        }

        public IEnumerable<ApofasiStegasiGridViewModel> Read()
        {
            var data = (from d in entities.ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ
                        orderby d.ΣΧΟΛΙΚΟ_ΕΤΟΣ descending, d.ΗΜΝΙΑ_ΣΥΝΤΑΞΗ descending, d.ΣΧΟΛΗ
                        select new ApofasiStegasiGridViewModel
                        {
        ...
        public void Create(ApofasiStegasiGridViewModel data)
        {
            ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ entity = new ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ()
            {
            ...
            entities.ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ.Add(entity);
            entities.SaveChanges();

            data.ΑΠΟΦΑΣΗ_ΚΩΔ = entity.ΑΠΟΦΑΣΗ_ΚΩΔ;
        }

        public void Update(ApofasiStegasiGridViewModel data)
        {
            ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ entity = entities.ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ.Find(data.ΑΠΟΦΑΣΗ_ΚΩΔ);
        ...
        public void Destroy(ApofasiStegasiGridViewModel data)
        {
            ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ entity = entities.ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ.Find(data.ΑΠΟΦΑΣΗ_ΚΩΔ);
            try
            {
                if (entity != null)
                {
                    ...
                    entities.ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ.Remove(entity);
                    entities.SaveChanges();
                }
            }
            catch { }
        ...
        public ApofasiStegasiGridViewModel Refresh(int entityId)
        {
            return entities.ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ.Select(d => new ApofasiStegasiGridViewModel
            {
                ΑΠΟΦΑΣΗ_ΕΙΔΟΣ = d.ΑΠΟΦΑΣΗ_ΕΙΔΟΣ,
                ΑΠΟΦΑΣΗ_ΚΩΔ = d.ΑΠΟΦΑΣΗ_ΚΩΔ,
                ΔΙΑΧΕΙΡΙΣΤΗΣ = d.ΔΙΑΧΕΙΡΙΣΤΗΣ,
        ...
        public ApofasiStegasiViewModel GetRecord(int entityId)
        {
            var data = (from d in entities.ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ
        ...
        public void UpdateRecord(ApofasiStegasiViewModel data, int apofasiId)
        {
            ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ entity = entities.ΑΠΟΦΑΣΗ_ΣΤΕΓΑΣΗ.Find(apofasiId);
