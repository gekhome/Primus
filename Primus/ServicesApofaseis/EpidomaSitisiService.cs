...
{
    public class EpidomaSitisiService : IEpidomaSitisiService, IDisposable
    {
        private readonly PrimusDBEntities entities;

        public EpidomaSitisiService(PrimusDBEntities entities)
        {
            this.entities = entities;
        }

        public IEnumerable<EpidomaSitisiViewModel> Read(EpidomaParameters ep)
        {
            List<EpidomaSitisiViewModel> data = new List<EpidomaSitisiViewModel>();

            if (ep.apofasiId > 0)
            {
                data = (from d in entities.ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ
                        where d.ΑΠΟΦΑΣΗ_ΚΩΔ == ep.apofasiId
                        orderby d.ΜΑΘΗΤΗΣ_ΕΠΩΝΥΜΟ, d.ΜΑΘΗΤΗΣ_ΟΝΟΜΑ, d.ΜΑΘΗΤΗΣ_ΤΑΞΗ
                        select new EpidomaSitisiViewModel
                        {
        ...
        public void Create(EpidomaSitisiViewModel data, EpidomaParameters ep, AitisiViewModel aitisi)
        {
            var apofasi = (from d in entities.ΑΠΟΦΑΣΗ_ΣΙΤΙΣΗ where d.ΑΠΟΦΑΣΗ_ΚΩΔ == ep.apofasiId select d).FirstOrDefault();

            ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ entity = new ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ()
            {
            ...
            entities.ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ.Add(entity);
            entities.SaveChanges();

            data.ΕΠΙΔΟΤΗΣΗ_ΚΩΔ = entity.ΕΠΙΔΟΤΗΣΗ_ΚΩΔ;
        }

        public void Update(EpidomaSitisiViewModel data, EpidomaParameters ep)
        {
            ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ entity = entities.ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ.Find(data.ΕΠΙΔΟΤΗΣΗ_ΚΩΔ);
        ...
        public void Destroy(EpidomaSitisiViewModel data)
        {
            ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ entity = entities.ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ.Find(data.ΕΠΙΔΟΤΗΣΗ_ΚΩΔ);
            ...
            if (entity != null)
            {
                ...
                entities.ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ.Remove(entity);
                entities.SaveChanges();
            }
        }

        public EpidomaSitisiViewModel Refresh(int entityId)
        {
            return entities.ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ.Select(d => new EpidomaSitisiViewModel
            {
                ΕΠΙΔΟΤΗΣΗ_ΚΩΔ = d.ΕΠΙΔΟΤΗΣΗ_ΚΩΔ,
                ΑΠΟΦΑΣΗ_ΕΙΔΟΣ = d.ΑΠΟΦΑΣΗ_ΕΙΔΟΣ,
                ΑΠΟΦΑΣΗ_ΚΩΔ = d.ΑΠΟΦΑΣΗ_ΚΩΔ,
        ...
        public EpidomaSitisiViewModel GetRecord(int entityId)
        {
            var data = (from d in entities.ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ
        ...
        public void UpdateRecord(EpidomaSitisiViewModel data, int epidotisiId)
        {
            ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ entity = entities.ΕΠΙΔΟΜΑ_ΣΙΤΙΣΗ.Find(epidotisiId);
